...
void ObjetSimuleMSS::initMeshObjet()
{
    ...
    // for(unsigned int i=0; i<_VISize; i+=3) {
    //     unsigned int a= m_ObjetSimule.vertex( P[i].x, P[i].y, P[i].z );
    //     unsigned int b= m_ObjetSimule.vertex( P[i+1].x, P[i+1].y, P[i+1].z );
    //     unsigned int c= m_ObjetSimule.vertex( P[i+2].x, P[i+2].y, P[i+2].z );
    //     m.triangle(a, b, c);
    // }
 ...
 */
void ObjetSimuleMSS::Simulation(Vector gravite, float viscosite, int Tps, const float &radius_sphere)
{
