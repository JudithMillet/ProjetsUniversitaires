...
int Viewer::render( )
{
    ...
    Transform t = Translation(0.5, 0, 0) * Scale(10,10,10) * Identity();
    ...
    Point pmin, pmax;
	m_sphere.bounds(pmin, pmax);
	Point pmin_t = t(pmin);
	Point pmax_t = t(pmax);

	radius_sphere = sqrt((pmin_t.x - pmax_t.x) * (pmin_t.x - pmax_t.x) +
						(pmin_t.y - pmax_t.y) * (pmin_t.y - pmax_t.y) +
						(pmin_t.z - pmax_t.z) * (pmin_t.z - pmax_t.z));
    ...
    if(key_state('m')) {
        _Simu->Interaction(Vector(mx, my, 0));
    }
...
int Viewer::update( const float time, const float delta )
{
    ...
    _Simu->Simulation(Tps, radius_sphere);
